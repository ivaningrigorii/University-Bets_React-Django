...
const GamerCells = ({ id_game, stat, }) => {
    ...
    const get_gamers = async () => {
        ...
        return axios.get("api/game/gamer/" + id_game + "/", {
            headers: {
    ...
    const handleDel = async (id) => {
        ...
        return axios.delete(("api/game/gamer/other/" + id + "/"), {
            headers: {
                'Authorization': 'Bearer ' + token,
            },
        })
    ...
    return (
        ...
        >
            {(gamers && gamers.length == 0) &&
                ...
                <Stack sx={{ fontSize: { xs: 11, sm: 12, md: 14 }, }} justifyContent="flex-start">
                    {gamers && gamers.map(gamer =>
                        <Stack direction="row" justifyContent="center" alignItems="baseline">
                            ...
                            {gamer.team.is_own == true &&
                                <Button onClick={() => handleDel(gamer.id)}>
                                    Убрать
                                </Button>
                            }
                        </Stack>
